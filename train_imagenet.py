...
def build_imagenet(model_state_dict, optimizer_state_dict, **kwargs):
    ...
    print('Found %d in training data', int(np.floor(len(train_data)*0.15)))
    #------------------------------------------------ steps -------------------------------------------------
    args.steps = int(np.ceil(int(np.floor(len(train_data)*0.15)) / (args.batch_size))) * torch.cuda.device_count() * args.epochs
    #---------------------------------------------------------------------------------------------------------
    ...
    train_queue = DataLoaderX(
        train_data, batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=args.num_workers,
...
if __name__=='__main__':
    ...
    args.data = '/data/datasets/imagenet2012/'
    ...
    args.solution = [[0, 1, 2, 1, 0, 0, 3, 1, 1, 1, 1, 3, 1, 1, 1, 0, 1, 3, 1, 1, 1, 0, 0, 0, 1], [1, 1, 2, 1, 1, 0, 7, 1, 0, 1, 1, 3, 0, 1, 1, 1, 0, 2, 0, 1, 0, 0, 0, 0, 8]]
