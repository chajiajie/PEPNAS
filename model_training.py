...
def Model_train(train_queue, model, train_criterion, optimizer, scheduler, args,valid_queue,eval_criterion,current_gen,print_ = False,flag=False):

    since_time = time.time()

    global_step = 0
    total = len(train_queue)
    pre_epoch = args.search_epochs+current_gen-1
    ...
    for epoch in range(pre_epoch):
        ...
        for step, (inputs, targets) in enumerate(train_queue):
            print('\r[Epoch:{0:>2d}/{1:>2d}, Training {2:>2d}/{3:>2d}, used_time {4:.2f}min]'.format(epoch+1, pre_epoch,step + 1, total, (time.time()-since_time)/60), end='')
        ...
        if print_ or (epoch+1)==pre_epoch:
    ...
    return top1.avg, top5.avg, objs.avg, valid_top1_acc, valid_top5_acc, loss, used_time , model.state_dict()
...
def solution_evaluation(model, train_queue, valid_queue,args,cur_gen,flag=False):
    ...
    train_criterion, eval_criterion, optimizer, scheduler = build_search_Optimizer_Loss(model, args, pre_epoch, epoch=-1)
    # ============================================ training the individual model and get valid accuracy ============================================
    result = Model_train(train_queue, model, train_criterion, optimizer, scheduler, args, valid_queue, eval_criterion,print_=False,current_gen=cur_gen,flag=flag)#True
    #top1.avg, top5.avg, objs.avg, valid_top1_acc, valid_top5_acc, loss, used_time
    Flops = Calculate_flops(model)

    return 1-result[3]/100, num_parameters, Flops,result[6],result[7]
