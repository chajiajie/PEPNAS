...
def build_search_cifar100(args, ratio=0.9,cutout_size=None, autoaugment=False,num_workers = 10):
    ...
    valid_data = dataset.CIFAR100(root=args.data, train=True, download=True, transform=valid_transform)
...
def build_search_Optimizer_Loss(model, args, pre_epoch, epoch=-1):
    model.cuda()
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, pre_epoch, args.search_lr_min, epoch)
