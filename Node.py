...
class Node(nn.Module):
    def __init__(self, previous_shape, dag_node, channels, cell_id, total_layers,stride=1,
        ...
        self.node_name = Operations_11_name[dag_node.op_id]#获取节点的操作名称
        self.Operation = Operations_11#获取节点的操作
        self.previous_shape = previous_shape#先前的shape
        self.linked_previous = [x for i,x in enumerate(previous_shape) if self.adj_node[i]==1]#如果有连接，获取之前shape

        self.channels = channels#16
        self.stride = stride#noraml:1,reduction:2
        ...
        self.Factor_flag = any(self.adj_node[2:])#判断连接是否全为0，是返回False
...
class NetworkCIFAR(nn.Module):
    def __init__(self, args, classes, layers,channels,
                 dag, use_aux_head, keep_prob,steps,drop_path_keep_prob, channels_double=False):#channels=16,layers=1
        super(NetworkCIFAR,self).__init__()
        self.args = args
        self.classes = classes
        self.layers = layers  # each Normal Cell repeat times  1
        self.channels = channels
